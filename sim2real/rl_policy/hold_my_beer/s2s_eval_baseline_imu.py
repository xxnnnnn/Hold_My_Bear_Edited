...
class S2SEvalPolicy(DecLocomotionPolicy):
    def __init__(self, config, model_path, rl_rate=50, policy_action_scale=0.25):
        ...
        self.ee_imu_acc = np.zeros((1, 3))
        self.ee_imu_gyro = np.zeros((1, 3))
    ...
    def _get_ee_imu_acc(self, robot_state_data):
        """
        Get end-effector IMU acceleration data from state_processor.
        Data is transmitted via DDS on rt/ee_imu channel.
        """

        if hasattr(self.state_processor, 'ee_imu_acc'):
            return self.state_processor.ee_imu_acc.reshape(1, 3)
        return np.zeros((1, 3))

    def _get_ee_imu_gyro(self, robot_state_data):
        """
        Get end-effector IMU gyroscope data from state_processor.
        Data is transmitted via DDS on rt/ee_imu channel.
        """
        if hasattr(self.state_processor, 'ee_imu_gyro'):
            return self.state_processor.ee_imu_gyro.reshape(1, 3)
        return np.zeros((1, 3))
    ...
    def rl_inference(self, robot_state_data):
        ...
        if self._inference_count == 1 or self._inference_count % 100 == 0:
            ...
            # Debug: Print EE IMU data (as sent to policy)
            obs_buffer_dict = self.get_current_obs_buffer_dict(robot_state_data)
            obs_ee_acc = obs_buffer_dict.get("ee_imu_acc", None)
            obs_ee_gyro = obs_buffer_dict.get("ee_imu_gyro", None)
            if obs_ee_acc is not None and obs_ee_gyro is not None:
                # Print 6D IMU vector: [acc_x, acc_y, acc_z, gyro_x, gyro_y, gyro_z]
                self.logger.info(f"EE IMU 6D (to policy): acc=[{obs_ee_acc[0, 0]:.4f}, {obs_ee_acc[0, 1]:.4f}, {obs_ee_acc[0, 2]:.4f}], gyro=[{obs_ee_gyro[0, 0]:.4f}, {obs_ee_gyro[0, 1]:.4f}, {obs_ee_gyro[0, 2]:.4f}]")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str,default="models/hold_my_beer/imu_14000.onnx", help="path to the ONNX model file")
